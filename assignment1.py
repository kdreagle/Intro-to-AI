...
class Puzzle:
    ...
    def build_gui(self, frame):
        ...
        self.task7_button = Button(frame, text="Task 7", fg="Brown",
                                   command=lambda: self.task7(frame,
                                                              int(input("Enter population size (even number): "))))
    ...
    #creates the population and make them produce children.
    # Those children produce with each other too until there is only one left.
    def task7(self, frame, population_size):
        ...
        while population_size > 1:
            ...
            for i in range(0, round(population_size/2)):
                children.append(generate_child(self, population[i], population[i+round(population_size/2)]))
            population = children
            population_size = len(population)
            print(population)
...
def generate_child(self, parent1, parent2):
    ...
    if(random() < 0.5):
        ...
        for i in range(0, n):
            crossover = random()
            mutation = round(n * crossover)
            for j in range(0, round(n/2)):
                if j == mutation:
                    max_jump = max(n - i, i - 1, n - j, j - 1)
                    new_jump = randint(1, max_jump)
                    if child[i - 1][j - 1] == new_jump:
                        new_jump = randint(1, max_jump)
                    child[i][j] = new_jump
                else:
                    child[i][j] = other_parent[i][j]
